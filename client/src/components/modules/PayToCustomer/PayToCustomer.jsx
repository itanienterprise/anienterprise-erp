...
const PayToCustomer = ({ addNotification, currentUser: propCurrentUser, refreshPendingIndicators, highlightId, isRequestedNotif }) => {
    ...
    const fetchEmployees = async () => {
        try {
            ...
            console.error('Error fetching employees map in PayToCustomer:', error);
        }
    };
            ...
            const decryptedBanks = rawData.map(bank => {
                ...
                if (bank && bank.data) {
                    if (typeof bank.data === 'string') {
                        try { d = { ...decryptData(bank.data), _id: bank._id }; } catch(e){}
                    } else if (typeof bank.data === 'object') {
                        d = { ...bank.data, _id: bank._id };
                    }
                }
    ...
    const getCustomerFinalBalance = (c) => {
        ...
        const paymentsHistoryList = (c.paymentHistory || []).filter(p => (p.status || '').toLowerCase() !== 'requested').map(p => ({
            ...p,
            type: 'payment',
        ...
        <div className="space-y-6">
            ...
            {showDeleteConfirm && typeof document !== 'undefined' && document.body && createPortal(
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[9999] flex items-center justify-center p-4">
                    <div className="bg-white max-w-sm w-full p-6 rounded-2xl shadow-xl text-center space-y-4 relative z-10 animate-in zoom-in duration-200">
