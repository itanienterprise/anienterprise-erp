...
function PI({
    ...
    const [formData, setFormData] = useState({
        ...
        grandTotalQuantity: '',
        showIndHsCode: false,
        productsList: [{ productName: '', hsCode: '', quantity: '', rate: '', amount: '', freight: '', totalFreight: '', showIndHsCode: false }],
        invoiceStyle: 'Style 1 SAA',
        port: '',
        placeOfReceipt: '',
        portOfLoading: 'ANY PLACE OF INDIA',
    ...
    useEffect(() => {
        try {
            localStorage.removeItem('pi_showIndHsCode_default');
        } catch {}
    }, []);
    ...
    const handleProductFieldChange = (idx, field, value) => {
        setFormData(prev => {
            ...
            if (idx === 0) {
                ...
                updated.showIndHsCode = list[0].showIndHsCode === true;
            }
                ...
                if (ip) {
                    ...
                    if (alreadySelected) {
                        ...
                        if (currentProducts.length === 0) {
                            currentProducts.push({ productName: '', hsCode: '', quantity: '', rate: '', amount: '', freight: '', totalFreight: '', showIndHsCode: false });
                        }
    ...
    const getEmptyFieldsForPi = (data) => {
        ...
        check(data.validityDate, 'Validity Date');
        check(data.piNumber, 'PI Number');
        if ((!data.ipNumbers || data.ipNumbers.length === 0) && (!data.ipNumber || String(data.ipNumber).trim() === '')) {
            empty.push('IP Search & Selection');
        }
        ...
        check(data.partyName, 'Importer / Party Name');
        ...
        check(data.finalDestination, 'Final Destination / Port');
        check(data.countryOrigin, 'Country of Origin');
        check(data.countryFinalDest, 'Country of Final Destination');
        check(data.indianBank, 'Advising Bank (Indian Bank)');
        ...
        if (products.length === 0) {
            ...
            products.forEach((p, idx) => {
                ...
                if (!p.quantity || String(p.quantity).trim() === '' || parseFloat(p.quantity) <= 0) {
                    empty.push(`${prefix}Quantity`);
                }
                if (!p.rate || String(p.rate).trim() === '' || parseFloat(p.rate) <= 0) {
        ...
        check(data.certification, 'Certification');
        const certList = (data.certification || '').split(',').map(s => s.trim().toLowerCase());
        if (!certList.includes('safta')) {
            empty.push('SAFTA');
        }
        check(data.packingType, 'Packing Type');
        check(data.invoiceStyle, 'Invoice Style');

        // Bank Details
        check(data.bankName, 'Issuing Bank');
        check(data.bankBranch, 'Branch');
        check(data.bankAccount, 'Account Number');
        check(data.bankMargin, 'Margin');

        // Description, Terms & Declaration
        check(data.descriptionGoods, 'Description of Goods');
        check(data.termsDeliveryPayment, 'Terms of Delivery and Payment');
        check(data.declaration, 'Declaration');
        check(data.status, 'Status');

        return empty;
    };

    const executeSubmit = async (finalPiNumbers) => {
    ...
    const handleSubmit = async (e) => {
        ...
        const emptyFields = getEmptyFieldsForPi(formData);
    ...
    const handleEdit = (record) => {
        const loadedList = record.productsList && record.productsList.length > 0
            ? record.productsList.map(p => ({ ...p, showIndHsCode: p.showIndHsCode !== undefined ? p.showIndHsCode === true : false }))
            : [{
                ...
                showIndHsCode: record.showIndHsCode !== undefined ? record.showIndHsCode === true : false
            }];
    ...
    const handleDelete = (id) => {
        if (!canDelete) {
            alert('Forbidden: You do not have permission to delete PI records');
            return;
        }
        if (onDeleteConfirm) {
    ...
    function getPiProductsList(pi) {
        if (pi.productsList && pi.productsList.length > 0) {
            return pi.productsList.map(p => ({ ...p, showIndHsCode: p.showIndHsCode !== undefined ? p.showIndHsCode === true : false }));
        }
        return [{
                                                ...
                                                <div className="flex items-center gap-6 px-4 py-2 bg-white border border-gray-200 rounded-lg h-[42px]">
                                                    <label className="inline-flex items-center gap-2 cursor-pointer text-sm font-medium text-gray-700">
                                                        <input
                                                            ...
                                                            checked={item.showIndHsCode === true}
