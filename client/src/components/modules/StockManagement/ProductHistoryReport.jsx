...
const ProductHistoryReport = ({
    ...
    const unifiedHistory = (() => {
        ...
        const damages = Object.values(damageHistory.reduce((acc, d) => {
                                        ...
                                        <tbody className="divide-y divide-gray-900">
                                            {purchaseHistory.map((item, idx) => (
                                                <tr key={idx} className="border-b border-gray-900 last:border-0 hover:bg-gray-50 transition-colors">
                                                    ...
                                                    <td className="px-2 py-1 text-[12px] text-right text-rose-600 font-bold">{Math.round(item.itemShortageQty || 0).toLocaleString('en-US')} {item.unit}</td>
                                                </tr>
                                            ))}
                                        </tbody>
                                        <tfoot>
                                            <tr className="bg-gray-100 border-t-2 border-gray-900 font-black">
                                                ...
                                                <td className="px-2 py-1.5 text-[12px] text-right border-r border-gray-900">
                                                    {Math.round(purchaseHistory.reduce((sum, i) => sum + (parseFloat(i.itemQty) || 0), 0)).toLocaleString('en-US')} {purchaseHistory[0]?.unit}
                                                </td>
                                                <td className="px-2 py-1.5 text-[12px] text-right border-r border-gray-900 text-blue-600">
                                                    {Math.round(purchaseHistory.reduce((sum, i) => sum + (parseFloat(i.itemInHouseQty) || 0), 0)).toLocaleString('en-US')} {purchaseHistory[0]?.unit}
                                                </td>
                                                <td className="px-2 py-1.5 text-[12px] text-right text-rose-600">
                                                    {Math.round(purchaseHistory.reduce((sum, i) => sum + (parseFloat(i.itemShortageQty) || 0), 0)).toLocaleString('en-US')} {purchaseHistory[0]?.unit}
                                                </td>
                                            </tr>
                                        </tfoot>
                                    </table>
                                                ...
                                                <tbody className="divide-y divide-gray-900">
                                                    {saleHistory.map((sale, idx) => (
                                                        <tr key={idx} className="border-b border-gray-900 last:border-0 hover:bg-gray-50 transition-colors">
                                                            ...
                                                            <td className="border-r border-gray-900 px-2 py-1 text-[12px] text-right text-gray-900">{sale.itemPacket.toLocaleString('en-US')}</td>
                                                            <td className="border-r border-gray-900 px-2 py-1 text-[12px] text-right text-gray-900 font-bold">{sale.itemQty.toLocaleString('en-US')} kg</td>
                                                            <td className="border-r border-gray-900 px-2 py-1 text-[12px] text-right text-gray-900">৳{sale.itemPrice.toLocaleString('en-IN')}</td>
                                                            <td className="px-2 py-1 text-[12px] text-right text-blue-600 font-bold">৳{sale.itemTotal.toLocaleString('en-IN')}</td>
                                                        </tr>
                                        ...
                                        <div key={idx} className="rounded-xl border border-blue-200 bg-blue-50/20 p-3 text-xs">
                                            ...
                                            <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                                                ...
                                                {isFruitCategory ? (<>
                                                    ...
                                                    <div className="text-gray-500">BAG</div><div className="font-medium text-gray-800 text-right">{sale.itemPacket?.toLocaleString('en-US')}</div>
                                                </>)}
                                                <div className="text-gray-500">Qty</div><div className="font-bold text-gray-900 text-right">{sale.itemQty?.toLocaleString('en-US')} kg</div>
                                                <div className="text-gray-500">Price</div><div className="font-medium text-gray-600 text-right">৳{sale.itemPrice?.toLocaleString('en-IN')}</div>
                                                <div className="text-blue-600 font-bold">Total</div><div className="font-black text-blue-700 text-right">৳{sale.itemTotal?.toLocaleString('en-IN')}</div>
                                            </div>
                                        </div>
                                    ))}
                                    <div className="rounded-xl border-2 border-gray-900 bg-gray-100 p-3 text-xs font-black">
                                        ...
                                        <div className="grid grid-cols-2 gap-x-3 gap-y-1">
                                            <div className="text-gray-700">Qty</div><div className="text-right">{saleHistory.reduce((sum, s) => sum + s.itemQty, 0).toLocaleString('en-US')} kg</div>
                                            <div className="text-blue-700">Amount</div><div className="text-blue-700 text-right">৳{saleHistory.reduce((sum, s) => sum + s.itemTotal, 0).toLocaleString('en-IN')}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="border border-gray-200 p-4 md:p-5 rounded-2xl bg-gray-50 shadow-sm print:border-gray-200 w-full md:min-w-[350px] md:max-w-[450px]">
                                    ...
                                    <div className="space-y-1">
                                        <div className="flex justify-between text-xs md:text-sm">
                                            ...
                                            <span className="font-bold">{Math.round(purchaseHistory.reduce((sum, i) => sum + (parseFloat(i.itemQty) || 0), 0)).toLocaleString('en-US')} kg</span>
                                        </div>
                                        <div className="flex justify-between text-xs md:text-sm">
                                            <span className="text-gray-600">Total Short:</span>
                                            <span className="font-bold text-rose-600">{Math.round(purchaseHistory.reduce((sum, i) => sum + (parseFloat(i.itemShortageQty) || 0), 0)).toLocaleString('en-US')} kg</span>
                                        </div>
                                        {activeTab === 'total' && (
                                            <div className="flex justify-between text-xs md:text-sm">
                                                <span className="text-gray-600">Total Sale:</span>
                                                <span className="font-bold">{saleHistory.reduce((sum, s) => sum + s.itemQty, 0).toLocaleString('en-US')} kg</span>
                                            </div>
                                        )}
