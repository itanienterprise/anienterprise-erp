...
const InsurancePayment = ({ currentUser: propCurrentUser, addNotification, highlightId, isRequestedNotif }) => {
    ...
    const canPaymentRequest = hasPermission(currentUser, 'insurancePayment', 'paymentRequest') || canAdd;
    const canApprove = isAdmin || isIncharge || hasPermission(currentUser, 'insurancePayment', 'special');
    const canViewPaymentRequest = hasPermission(currentUser, 'insurancePayment', 'paymentRequest') || hasPermission(currentUser, 'insurancePayment', 'special') || canApprove;
    const canShowEntryBy = isAdmin || isIncharge || hasPermission(currentUser, 'insurancePayment', 'showEntryBy');
    const canManage = canAdd || canEdit || canDelete || canApprove || canPaymentRequest;
    ...
    const requestedCount = React.useMemo(() => {
        return payments.filter(p => p.status === 'Requested').length;
    }, [payments]);
    ...
    const fetchEmployees = async () => {
        try {
            ...
            console.error('Error fetching employees map in InsurancePayment:', error);
        }
    };
    ...
    const handleAddPayment = async (e) => {
        ...
        try {
            const initialStatus = isEditMode ? (editingPayment?.status || 'Adjusted') : 'Requested';
            const paymentData = {
                ...newPayment,
                ...
                <div className="flex items-center justify-center md:justify-end gap-2 w-full md:w-auto">
                    {!showAddModal && (
                        ...
                        <button
                            ...
                            className="h-10 flex items-center justify-center gap-2 px-4 rounded-xl border border-gray-200 bg-white text-gray-600 hover:bg-gray-50 transition-all active:scale-95 text-sm font-medium shadow-sm"
                        >
                ...
                <div className="relative group mb-8">
                    ...
                    <div className="relative bg-white/80 backdrop-blur-xl rounded-[2rem] border border-white shadow-2xl animate-in slide-in-from-top-4 duration-500">
                        <div className="px-8 py-6 border-b border-gray-100/50 flex items-center justify-between bg-gradient-to-r from-gray-50/50 to-white rounded-t-[2rem]">
                            <div>
                                <h3 className="text-lg font-black text-gray-900 tracking-tight">{isEditMode ? 'Edit Insurance Payment' : (isRequestMode ? 'New Payment Request' : 'New Insurance Payment')}</h3>
                            ...
                            <table className="w-full text-left text-[13px]">
                                <thead className="bg-gray-50/50 border-b border-gray-200">
                                    <tr>
                                        ...
                                        {canShowEntryBy && <th className="px-4 py-3 font-bold text-gray-500 text-xs uppercase tracking-wider text-center">Entry By</th>}
                                        {canManage && <th className="px-4 py-3 font-bold text-gray-500 text-xs uppercase tracking-wider text-center">Actions</th>}
                                    </tr>
                                </thead>
                                <tbody className="divide-y divide-gray-100">
                                    {isLoading ? (
                                        <tr><td colSpan={10 + (canManage ? 1 : 0) + (canShowEntryBy ? 1 : 0)} className="px-4 py-12 text-center text-gray-400">Loading payments...</td></tr>
                                    ) : filteredPayments.length === 0 ? (
                                        <tr><td colSpan={10 + (canManage ? 1 : 0) + (canShowEntryBy ? 1 : 0)} className="px-4 py-12 text-center text-gray-400">No payment records found.</td></tr>
                                    ) : (
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <td className="px-4 py-3 whitespace-nowrap text-center">
                                                        {p.status === 'Requested' ? (
                                                            <span className="px-2.5 py-0.5 rounded-lg text-[10px] font-bold uppercase tracking-wider border bg-amber-50 text-amber-700 border-amber-200/80 shadow-xs">
                                                                Requested
                                                            </span>
                                                        ) : p.isAdjustReturn ? (
                                                    ...
                                                    {canShowEntryBy && (
                                                        <td className="px-4 py-3 whitespace-nowrap text-center text-xs font-semibold text-gray-600">
                                                            {getEntryByName(p.entryBy, p.entryByName)}
                                                        </td>
                                                    )}
                                                    {canManage && (
                                                        <td className="px-4 py-3 whitespace-nowrap text-center">
                                                            <div className="flex items-center justify-center gap-2">
                                                                ...
                                                                {canEdit && (
                                                                    <button onClick={() => handleEditPayment(p)} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title="Edit"><EditIcon className="w-4 h-4" /></button>
                                                                )}
                                                                {p.status !== 'Requested' && canDelete && (
                                                                    <button onClick={() => handleDeletePayment(p)} className="p-1.5 text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete"><TrashIcon className="w-4 h-4" /></button>
                                                                )}
                                                            </div>
                                                        </td>
                                                    )}
                                    ...
                                    <div key={p._id} className={`bg-white rounded-xl border transition-all duration-300 overflow-hidden ${isExpanded ? 'border-blue-200 shadow-md ring-1 ring-blue-50' : 'border-gray-100 shadow-sm hover:border-gray-200'}`}>
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-2 shrink-0">
                                                {p.status === 'Requested' ? (
                                                    <span className="px-2 py-0.5 rounded-lg text-[9px] font-bold uppercase tracking-wider border bg-amber-50 text-amber-700 border-amber-200">
                                                        Requested
                                                    </span>
                                                ) : p.isAdjustReturn ? (
                                                ...
                                                <div className="grid grid-cols-[125px_8px_1fr] gap-y-2 pt-3 text-xs items-baseline">
                                                    ...
                                                    {canShowEntryBy && (
                                                        <>
                                                            ...
                                                            <span className="text-gray-400 font-bold text-[10px]">:</span>
                                                            <span className="font-semibold text-gray-700 text-[11px]">{getEntryByName(p.entryBy, p.entryByName)}</span>
                                                        </>
                                                    )}
                                                        ...
                                                        <div className="col-span-3 flex gap-2 pt-3 mt-1 border-t border-gray-100 w-full">
                                                            {p.status === 'Requested' && canApprove && (
                                                                <>
                                                                    <button
                                                                        onClick={() => handleApprovePayment(p)}
                                                                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-emerald-600 text-white rounded-xl font-bold text-xs active:scale-95 transition-all shadow-sm"
                                                                    >
                                                                        <CheckIcon className="w-3.5 h-3.5" /> Accept
                                                                    </button>
                                                                    <button
                                                                        onClick={() => handleRejectPayment(p)}
                                                                        className="flex-1 flex items-center justify-center gap-1.5 py-2.5 bg-rose-600 text-white rounded-xl font-bold text-xs active:scale-95 transition-all shadow-sm"
                                                                    >
