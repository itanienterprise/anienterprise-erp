                    ...
                    {[
                        ...
                        <div key={i} className={`bg-white p-5 rounded-2xl border border-gray-100 shadow-sm transition-all hover:shadow-md hover:border-${card.color}-100 group ${card.border || ''}`}>
                            ...
                            <div className="flex items-baseline gap-1">
                                <span className={`text-xl font-black text-${card.color}-600`}>৳{card.value.toLocaleString()}</span>
                            </div>
                        </div>
                    ))}
                </div>
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
                                {[
                                    ...
                                    <div key={i} className={`p-4 bg-${stat.color}-50/50 border border-gray-100 rounded-2xl`}>
                                        ...
                                        <p className={`text-sm font-black text-${stat.color}-600`}>৳{stat.value.toLocaleString()}</p>
                                    </div>
                                ))}
                            </div>
                                                ...
                                                activeHistoryTab === 'payments' ? (
                                                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                                        ...
                                                        <td className="px-6 py-4 text-xs font-bold text-blue-600 text-right whitespace-nowrap">
                                                            {item.adjustedAmount > 0 ? `৳${item.adjustedAmount.toLocaleString()}` : '-'}
                                                        </td>
                                                        <td className="px-6 py-4 text-xs font-bold text-gray-700 text-right whitespace-nowrap">৳{parseFloat(item.amount || 0).toLocaleString()}</td>
                                                        <td className="px-6 py-4 text-xs font-black text-gray-900 text-right bg-gray-50/30 whitespace-nowrap">
                                                            ৳{((parseFloat(item.amount || 0)) + (parseFloat(item.adjustedAmount || 0))).toLocaleString()}
                                                        </td>
                                                    </tr>
                                                ) : (
                                                    <tr key={idx} className="hover:bg-gray-50/50 transition-colors">
                                                        ...
                                                        <td className="px-6 py-4 text-xs font-black text-rose-600 text-right">৳{parseFloat(item.netPremium || 0).toLocaleString()}</td>
                                                        <td className="px-6 py-4 text-xs font-black text-emerald-600 text-right">৳{parseFloat(item.expectedReturnAmount || 0).toLocaleString()}</td>
                                                    </tr>
                                                )
                                            ))
                                        ) : (
