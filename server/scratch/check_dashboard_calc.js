...
mongoose.connect('mongodb://127.0.0.1:27017/erp_db').then(async () => {
    ...
    console.log("=== CALCULATING STOCK FOR PRESENT (NO DATE FILTER) ===");
    const res = calculateStockData(stockRecords, { warehouse: '' }, '', warehouseData, salesRecords, products, damages);
    
    const group = res.displayRecords.find(g => g.productName.toUpperCase() === 'MAIZE');
    if (group) {
        console.log("Dashboard Calc (calculateStockData):");
        console.log(JSON.stringify(group, null, 2));
    } else {
        console.log("Product MAIZE not found in displayRecords");
    }
