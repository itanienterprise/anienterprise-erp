...
mongoose.connect('mongodb://127.0.0.1:27017/erp_db').then(async () => {
    ...
    console.log("=== HILI WAREHOUSE BRAN STOCK ===");
    const res = calculateStockData(stockRecords, { warehouse: 'HILI' }, '', warehouseData, salesRecords, products, damages);
    const group = res.displayRecords.find(r => r.productName === 'BRAN');
    if (!group) {
        console.log("BRAN group not found under HILI filter!");
    } else {
        console.log(`Product: ${group.productName} | InHouseQty: ${group.inHouseQuantity} | SaleQty: ${group.saleQuantity}`);
        group.brandList.forEach(b => {
            console.log(`  Brand: ${b.brand} | InHouseQty (Closing): ${b.inHouseQuantity} | Opening: ${b.openingQuantity} | Sale: ${b.saleQuantity} | Damage: ${b.damageQuantity} | Arrival: ${b.periodArrivalQuantity}`);
        });
    }
