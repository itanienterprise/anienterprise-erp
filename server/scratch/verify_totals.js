...
mongoose.connect('mongodb://127.0.0.1:27017/erp_db').then(async () => {
    ...
    console.log("=== VERIFYING GROUP TOTALS ===");
    res.displayRecords.forEach(group => {
        const sumBrandsInHouse = group.brandList.reduce((sum, b) => sum + b.inHouseQuantity, 0);
        const diff = Math.abs(group.inHouseQuantity - sumBrandsInHouse);
        console.log(`Product: ${group.productName.padEnd(15)} | Group inHouseQty: ${group.inHouseQuantity.toString().padStart(8)} | Sum of brands: ${sumBrandsInHouse.toString().padStart(8)} | Match: ${diff < 0.01 ? 'YES' : 'NO'}`);
        if (diff >= 0.01) {
            console.log("  Brands:");
            group.brandList.forEach(b => {
                console.log(`    ${b.brand}: ${b.inHouseQuantity}`);
            });
        }
    });

    console.log("\n=== VERIFYING SUMMARY TOTALS ===");
    const sumGroupsInHouse = res.displayRecords.reduce((sum, g) => sum + g.inHouseQuantity, 0);
    console.log(`Total In-House (Closing) Quantity in Summary: ${res.totalInHouseQty}`);
    console.log(`Sum of all groups' In-House Quantities      : ${sumGroupsInHouse}`);
    console.log(`Match                                        : ${Math.abs(res.totalInHouseQty - sumGroupsInHouse) < 0.01 ? 'YES' : 'NO'}`);
